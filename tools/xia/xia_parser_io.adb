...
--           else
--              get(buf(loc));
            Buf(Loc) := Element(User_Input, 1);
            Delete(User_Input, 1, 1);
--           end if;
...
--      when END_ERROR =>
   ...
   procedure Open_Input(Set_String : in String) is
   begin
      yy_init := true;
      User_Input := To_Unbounded_String(Set_String);
   end Open_Input;
...
--        end if;
      User_Input := Null_Unbounded_String;
   end Close_Input;
