...
--  $RCSfile: coldframe-events-standard.adb,v $
...
package body ColdFrame.Events.Standard is
   ...
   begin
      ...
      Set (The => The,
           On => On,
           To_Fire => To_Fire,
           At_Time => Create (Ada.Real_Time.Clock
   ...
   function Create (Time_For : Ada.Real_Time.Time) return Time_P is
   begin
      return new Real_Time'(The => Time_For);
   end Create;


   function Equivalent (For_Time : Real_Time) return Ada.Real_Time.Time is
   begin
      return For_Time.The;
   end Equivalent;


   function Create (Time_For : Ada.Calendar.Time) return Time_P is
   begin
      return new Calendar_Time'(The => Time_For);
   end Create;


   function Equivalent (For_Time : Calendar_Time) return Ada.Real_Time.Time is
      use type Ada.Calendar.Time;
      use type Ada.Real_Time.Time;
      Offset : constant Duration := For_Time.The - Ada.Calendar.Clock;
   begin
      return Ada.Real_Time.Clock + Ada.Real_Time.To_Time_Span (Offset);
   end Equivalent;
   ...
   begin
      ...
      loop
         ...
         if Timed_Event_Queues.Is_Empty (The_Events) then
            ...
            or
               delay until Equivalent
                 (Timed_Event_Queues.Front
                    (The_Events).Time_To_Fire.all);
               declare
   ...
   function "<" (L, R : Timer_Queue_Entry_P) return Boolean is
      use type Ada.Real_Time.Time;
   begin
      return Equivalent (L.Time_To_Fire.all) < Equivalent (R.Time_To_Fire.all);
   end "<";
