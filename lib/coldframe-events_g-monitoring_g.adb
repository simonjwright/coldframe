...
package body ColdFrame.Events_G.Monitoring_G is
   ...
   protected body Excluder is
      ...
      begin
         ...
         Current_Event_Started := Clock;
         if The_Event.all in Held_Event'Class then
            Current_Event_Record.Tag :=
              Held_Event (The_Event.all).The_Event.all'Tag;
            Current_Event_Record.Was_Held := True;
         else
            Current_Event_Record.Tag := The_Event.all'Tag;
            Current_Event_Record.Was_Held := False;
         end if;
      end Fetch;
      ...
      begin
         ...
         if Owner /= Monitoring_G.Event_Queue_Base (The_Queue.all)
           .The_Dispatcher'Identity
         then
            Ada.Exceptions.Raise_Exception
              (Exceptions.Use_Error'Identity,
